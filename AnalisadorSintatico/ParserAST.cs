    ...
    public class ParserAST
    {
        ...
        public ProgramNode Parse()
        {
            ...
            if (Match(TokenType.Var))
            {
                ...
                {
                    program.DeclaracaoVariaiveis.Add(DVar());
                }
            }

            while (!IsAtEnd())
            {
                program.StatementsPrograma.Add(Com());
            }
        ...
        private VarDeclarationNode DVar()
        {
            ...
            {
                Consume(TokenType.Identifier, "Expect identifier after type.").Lexeme
            };
        ...
        private ExpressionNode ExprR()
        {
            ...
            if (Match(TokenType.Equal, TokenType.NotEqual, TokenType.Less, TokenType.Greater, TokenType.LessEqual, TokenType.GreaterEqual))
            {
                ...
                var right = Expr();
                return new OperacaoBinariaNode(left, @operator, right);
            }
            return left;
        }
        ...
        private ExpressionNode Expr()
        {
            ...
            while (Match(TokenType.Plus, TokenType.Minus))
            {
                var @operator = Previous().Type;
                var right = Term();
                left = new OperacaoBinariaNode(left, @operator, right);
            }
            return left;
        }
